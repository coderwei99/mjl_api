...
const { create, verification, findCodeList } = require("../server/code.server");
...
class CodeController {
  ...
  async createCode(ctx, next) {
    ...
    const code = hash
      .update(Buffer.from(shop_name))
      .digest("hex")
      .toLocaleUpperCase();

    const result = await create(code, shop_name);
    if (result == "error")
      return ctx.app.emit("error", repetitionCodeError, ctx);
    ctx.body = {
      code: 200,
      data: result,
      message: "创建邀请码成功",
  ...
  async verificationCode(ctx) {
    ...
    console.log(code);
    if (!res || res.is_used) return ctx.app.emit("error", verifyCodeError, ctx);
    ctx.body = {
      code: 200,
      data: res,
      message: "邀请码正确",
  ...
  async getCodeList(ctx) {
    ...
    const res = await findCodeList({ pageNum, pageSize }, params);
    ctx.body = {
      code: 200,
      data: res,
      message: "获取邀请码列表",
